...
window.showGameToast = function(message) {
    ...
    toast.style.cssText = `
        ...
        border: 2px solid #ff0055 !important; color: #fff !important; padding: 10px 20px !important;
        border-radius: 25px !important; z-index: 999999999 !important; font-family: monospace !important;
        font-size: 0.85rem !important; box-shadow: 0 0 20px rgba(255,0,85,0.5) !important;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const dexElement = document.querySelector('.rot-dex') || document.getElementById('rot-dex');
    if (dexElement && typeof paranormalSpawns !== 'undefined') {
        dexElement.innerText = `PARANORMAL DEX (0/${paranormalSpawns.length})`;
    }
});
...
// Converts percentage (1-100) into visual 3-star string
...
window.inspectRot = function(index) {
    ...
    modal.innerHTML = `
        <div style="background: #110a1c; border: 2px solid ${qualityColor}; border-radius: 16px; padding: 30px; text-align: center; max-width: 350px; width: 100%; box-shadow: 0 0 30px rgba(0,0,0,0.8);">
            ...
            <div style="background: rgba(255,255,255,0.05); border-radius: 10px; padding: 12px; margin-bottom: 20px;">
                <div style="font-size: 1.1rem; font-weight: bold; color: ${qualityColor}; margin-bottom: 8px;">
                    Entity Power Rating: ${rot.quality || 50}%
                </div>
...
window.calculateRotStats = function(rot) {
    ...
    if (typeof paranormalSpawns !== 'undefined') {
        const dbChar = paranormalSpawns.find(c => c.name.toLowerCase().trim() === rot.name.toLowerCase().trim());
        if (dbChar) {
...
window.openCardDetails = function(inventoryIndex) {
    ...
    const charData = typeof paranormalSpawns !== 'undefined' 
        ? paranormalSpawns.find(c => c.name.toLowerCase().trim() === rot.name.toLowerCase().trim())
        : null;
    ...
    if (charData && charData.evolution) {
        ...
        evolveButtonHtml = `
            <button onclick="window.evolveRot(${inventoryIndex}); openCardDetails(${inventoryIndex});" style="width: 100%; background: ${canEvolve ? '#ff0055' : '#333'}; color: ${canEvolve ? '#fff' : '#aaa'}; font-weight: bold; padding: 8px; border: none; border-radius: 5px; cursor: ${canEvolve ? 'pointer' : 'not-allowed'}; font-family: monospace; font-size: 0.8rem; margin-bottom: 8px;">
    ...
    detailModal.innerHTML = `
        <div style="background: linear-gradient(180deg, #110a1c, ${rarityColor}44); border: 4px solid ${rarityColor}; border-radius: 20px; padding: 20px; width: 100%; max-width: 320px; text-align: center; box-shadow: 0 0 50px ${rarityColor}88; position: relative;">
            ...
            <div style="font-size: 0.75rem; color: #ffcc00; margin-bottom: 12px; font-weight: bold; cursor: pointer;" onclick="inspectRot(${inventoryIndex})">Power Rating: ${rot.quality || 50}% (Click to Inspect)</div>
            ...
            <div style="text-align: left; background: rgba(0,0,0,0.6); padding: 15px; border-radius: 10px; border: 1px solid #333; margin-bottom: 15px;">
                <div style="margin-bottom: 12px;">
                    <div style="display: flex; justify-content: space-between; font-size: 0.85rem; margin-bottom: 4px; font-weight: bold;">
                        <span style="color: #00ff80;">❤️ HEALTH</span>
                    ...
                    <div style="width: 100%; height: 12px; background: #222; border-radius: 6px; overflow: hidden; border: 1px solid #444;">
                        <div style="width: ${hpPercent}%; height: 100%; background: #00ff80; box-shadow: 0 0 8px #00ff80;"></div>
                    </div>
                </div>
            ...
            <div style="background: rgba(0,0,0,0.8); padding: 10px; border-radius: 10px; border: 2px dashed ${rarityColor};">
                <div style="font-size: 1rem; color: #fff; font-weight: bold; margin-bottom: 10px;">
                    🔮 Essence: <span style="color: #00ccff;">${candyCount}</span>
                </div>
                
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    ${evolveButtonHtml}
                    <div style="display: flex; justify-content: space-between; gap: 10px;">
                        <button onclick="levelUpRot(${inventoryIndex})" style="flex: 1; background: #ff0055; color: #fff; font-weight: bold; padding: 8px; border: none; border-radius: 5px; cursor: pointer;">
                            LEVEL UP<br><span style="font-size: 0.7rem;">(Cost: ${levelUpCost})</span>
                        </button>
                        <button onclick="transferRot(${inventoryIndex})" style="flex: 1; background: #333; color: #fff; font-weight: bold; padding: 8px; border: none; border-radius: 5px; cursor: pointer;">
                            RELEASE<br><span style="font-size: 0.7rem;">(+1 Essence)</span>
                        </button>
                    </div>
                </div>
            </div>
...
window.levelUpRot = function(index) {
    ...
    if ((playerData.candies[candyKey] || 0) >= cost) {
        ...
        showGameToast(`❌ Not enough essence! Need ${cost} essence.`);
    }
};
...
window.transferRot = function(index) {
    ...
    if (playerData.inventory.length <= 1) {
        showGameToast("🚨 You cannot release your very last entity!");
        return;
    }
    ...
    if (confirm(`⚠️ Are you sure you want to release this Lvl ${rot.level} ${rot.name} back to the spirit realm? You will receive 1 Essence. This CANNOT be undone.`)) {
        ...
        showGameToast(`🔮 Released ${rot.name} for 1 Essence!`);
    }
};